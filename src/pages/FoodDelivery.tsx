...
const FoodDelivery: React.FC = () => {
  ...
  const fetchVendors = async () => {
    try {
      ...
      // Adjust to match backend response { vendors: [...] }
      let vendorData: FoodVendorProfile[] = [];
      ...
      // Fix logo path (prepend base URL if needed)
      vendorData = vendorData.map(v => ({
        ...v,
        logo: v.logo?.startsWith('http')
          ? v.logo
          : `${import.meta.env.VITE_API_BASE_URL || 'http://127.0.0.1:8000'}/${v.logo}`,
      }));
